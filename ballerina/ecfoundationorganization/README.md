...
To use the `sap.successfactors.ecfoundationorganization` connector in your Ballerina application, modify the `.bal` file as follows:

### Step 1: Import the module

Import the `sap.successfactors.ecfoundationorganization` module.

```ballerina
import ballerinax/sap.successfactors.ecfoundationorganization as foundationorganization;
```
...
foundationorganization:Client foundationorganizationClient = check new (
    {
      auth: {
        username,
        password
...
```ballerina
foundationorganization:FOLegalEntityLocalUSAWrapper result = check foundationorganizationClient->listFOLegalEntityLocalUSAs();
```
