...
To use the `sap.successfactors.ectimeoff` connector in your Ballerina application, modify the `.bal` file as follows:

### Step 1: Import the module

Import the `sap.successfactors.ectimeoff` module.

```ballerina
import ballerinax/sap.successfactors.ectimeoff as timeoff;
```
...
timeoff:Client timeoffClient = check new (
    {
      auth: {
        username,
        password
...
```ballerina
timeoff:EmployeeTimeWrapper result = check timeoffClient->listEmployeeTimes();
```
