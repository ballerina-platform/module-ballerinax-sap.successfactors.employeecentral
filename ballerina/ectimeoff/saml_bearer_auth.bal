...
#
# Use this when your SAP SuccessFactors tenant has an OAuth2 client application registered
# under Admin Center > Manage OAuth2 Client Applications, as an alternative to Basic Authentication.
...
# Obtains an OAuth 2.0 access token from an SAP SuccessFactors tenant using the SAML 2.0 Bearer
# Assertion Flow ([RFC 7522](https://www.rfc-editor.org/rfc/rfc7522)).
#
...
# token endpoint for a Bearer access token, which is then used to authenticate this connector's
# requests.
#
# + config - The SAML Bearer authentication configuration
# + return - The OAuth 2.0 access token, or an error if assertion building, signing, or the
# token exchange failed
isolated function getSamlBearerAccessToken(SamlBearerAuthConfig config) returns string|error {
    ...
    string assertionXml = buildSamlAssertionXml(
    ...
    string signedAssertion = check signSamlAssertion(assertionXml, assertionId, certificateBase64, privateKey);
    ...
    if response.statusCode < 200 || response.statusCode >= 300 {
        return error("SAML Bearer token exchange failed", statusCode = response.statusCode, body = payload);
    }
...
isolated function buildSamlAssertionXml(string assertionId, string issueInstant, string notBefore,
...
#
# + assertionXml - The unsigned assertion XML, as built by `buildSamlAssertionXml`
...
isolated function signSamlAssertion(string assertionXml, string assertionId, string certificateBase64,
